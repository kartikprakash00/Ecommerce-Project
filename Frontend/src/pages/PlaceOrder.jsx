...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      switch (method) {
        ...
        // Card Payment
        case 'card':
          navigate('/card-checkout', { state: { orderData } }); // Navigate to card checkout
          break;
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[51.5vh] border-t'>
      {/* Left Side */}
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            <div onClick={() => setMethod('stripe')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'stripe' ? 'bg-[#5433FF]' : ''}`}></p>
          ...
          <div className='w-full text-end mt-8'>
            <button type='submit' className='bg-slate-200 text-black text-sm px-16 py-3 w-full active:bg-gray-700 hover:bg-black hover:text-white transition-all duration-300'>PLACE ORDER</button>
          </div>
